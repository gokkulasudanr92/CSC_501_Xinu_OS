...
struct lock_descriptor_table lentry[NLOCKS]; /* locks table */
int nextlock;		/* next lock slot to use in lcreate*/
struct	qent	q[NQENT];	/* q table (see queue.c)		*/
struct 	qent 	semrq[NLENT]; /* semrq table */
struct 	qent	semwq[NLENT]; /* semwq table */
...
LOCAL int sysinit()
{
	...
	for (i=0 ; i<NPROC ; i++) {	/* initialize process table */
		...
		for (j = 0; j < NLOCKS; j++) {
			proctab[i].locks_held[j][0] = 0;
			proctab[i].locks_held[j][1] = 0;
		}
	}
