...
LOCAL int sysinit()
{
	...
	/* intialize the system tracker struct */
	for (i = 0; i < NPROC; i++) {
		pptr = &proctab[i];
		for (j = 0; j < NSYS; j++) {
			pptr->sysdata[j].count = 0;
			pptr->sysdata[j].total_execution_time = 0;
		}
	}
	...
	*( (int *)pptr->pbase ) = MAGIC;
	...
	pptr->pprio = 0;
	currpid = NULLPROC;
	// The scheduler is not set, so no need for assigning quantum
	pptr->counter = 0;
